...
var server = http.createServer(function (request, response){
    ...
    } else if(request.url === '/ninjas_style.css') {
        fs.readFile('../public/css/ninjas_style.css', 'utf8', function (errors, contents){
            response.writeHead(200, {'Content-Type': 'text/css'});  // send data about response
            response.write(contents);  //  send response body
            response.end(); // finished!
        });
    } else if(request.url === '/dojosnew_style.css') {
        fs.readFile('../public/css/dojosnew_style.css', 'utf8', function (errors, contents){
            response.writeHead(200, {'Content-Type': 'text/css'});  // send data about response
            response.write(contents);  //  send response body
            response.end(); // finished!
        });
